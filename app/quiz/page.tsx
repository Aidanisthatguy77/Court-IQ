  ...
  return (
    <div className="space-y-6">
      ...
      <Card title={`Scenario ${index + 1}`} subtitle={`Score: ${score}/${quizQuestions.length}`}>
        ...
        <div className="space-y-2">{q.options.map((opt, i) => <button key={opt} className="block w-full rounded-lg border border-court-line bg-court-panelSoft p-3 text-left text-sm hover:border-court-accent" onClick={() => choose(i)}>{opt}</button>)}</div>
